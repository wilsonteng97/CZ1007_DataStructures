...
void RecursiveReverse(ListNode **ptrHead)
{
    ...
    *ptrHead = rest;
    RecursiveReverse(ptrHead); // &rest does not work cos otherwise, **ptrhead would not be updated
...
//        return;
//
//    RecursiveReverse(&rest); //  Works because rest is updated but start is not. Then *ptrHead is updated to the new rest every iteration afterwards.
//
//    start->next->next = start;
//    start->next = NULL;
//
